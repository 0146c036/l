    ...
    try:
        ...
        for event in events:
            ...
            if (
                ...
                logger.info(
                    f"Received webhook message from {user_id}: '{message_text}'"
                )
                ...
                if message_text.lower() in [
                    ...
                    reply_message(
                        reply_token,
                        "已為您清除暫存的對話狀態。您可以隨時輸入新行程，例如：「明天下午3:00在817教室與組員開會」"
                    )
                ...
                if missing_fields:
                    ...
                    try:
                        ...
                        reply_message(
                            reply_token,
                            "⚠️ 日期或時間格式解析出錯，請輸入「取消」重新嘗試，例如：「本週一下午4:00在延平817開會」"
                        )
